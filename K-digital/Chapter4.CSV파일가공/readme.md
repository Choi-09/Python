### 3. CSV파일로 데이터 다루기
  ...
  + 3-4. csv파일 데이터 분석
    ...
    2) 라이브러리로 csv파일 열고 변수1에 넣기    → opencsv(파일명)
    3) 문자형숫자를 숫자형으로 전환(자릿수 표시','없어진다.) 후 변수2에 넣기     → .switch()
    4) 변수2 출력해서 확인하기  
    5)  빈 배열 만들고 배열안에 컬럼명 넣기      → 변수 = [['컬럼1', '컬럼2',...,'컬럼n']]
    6)  for문 돌면서 원하는 값을 append메소드를 사용해 한줄씩 추가한다.      → for i in 변수 
    7) 오류는 pass     → try-except
    8) append한 값을 새로운 파일에 쓰고 생성.      → .append()
    ```
